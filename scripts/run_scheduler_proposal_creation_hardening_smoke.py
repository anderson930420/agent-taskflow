#!/usr/bin/env python3
"""Run the J4 scheduler proposal creation hardening smoke.

This smoke proves the operator-gated Level 2 path from a live scheduler
candidate to recorded scheduler proposal evidence, then reads that evidence
back through the J2 helper and API. It intentionally does not create
confirmations, verifier reports, handoffs, runtime runs, executor runs,
validator runs, approvals, merges, cleanup records, background workers, or
GitHub mutations.
"""

from __future__ import annotations

import argparse
import json
import shutil
import sqlite3
import sys
import tempfile
from pathlib import Path
from typing import Any

REPO_ROOT = Path(__file__).resolve().parents[1]
if str(REPO_ROOT) not in sys.path:
    sys.path.insert(0, str(REPO_ROOT))

from fastapi.testclient import TestClient

from agent_taskflow.api.main import create_app
from agent_taskflow.models import TaskRecord
from agent_taskflow.scheduler_candidate_proposals import (
    SchedulerCandidateProposalRequest,
    create_scheduler_proposal_from_candidate,
)
from agent_taskflow.scheduler_proposal_readback import (
    list_task_scheduler_proposal_readbacks,
)
from agent_taskflow.scheduler_proposals import (
    PROPOSAL_ARTIFACT_TYPE,
    PROPOSAL_EVENT_TYPE,
)
from agent_taskflow.store import TaskMirrorStore
from agent_taskflow.tasks import normalize_task_key


DEFAULT_TASK_KEY = "AT-J4-PROPOSAL-SMOKE"
DEFAULT_PROJECT = "agent-taskflow"
EXPECTED_COMMAND_KIND = "create_task_execution_package"

FORBIDDEN_ARTIFACT_TYPES = (
    "scheduler_confirmation",
    "scheduler_confirmation_verifier_report",
    "verifier_report",
    "intake_runner_handoff",
    "runtime_handoff_execution",
    "validation_result",
    "approval_decision",
    "merge_recorded",
    "cleanup",
)

FORBIDDEN_EVENT_TYPES = (
    "scheduler_confirmation_created",
    "scheduler_confirmation_verifier_report",
    "verifier_report",
    "intake_runner_handoff_created",
    "runtime_preflight_finished",
    "runtime_execution_started",
    "runtime_execution_finished",
    "executor_run_started",
    "executor_run_finished",
    "validation_result",
    "approval_decision",
    "merge_recorded",
    "cleanup",
)

FORBIDDEN_PAYLOAD_MARKERS = (
    "approved_task_runner",
    "executor_run_started",
    "executor_run_finished",
    "validation_result",
    "runtime_execution_started",
    "runtime_execution_finished",
    "scheduler_confirmation_created",
    "intake_runner_handoff_created",
)

MISSION_CONTROL_SAFETY_TEXT = (
    "NOT execution permission",
    "Proposal is not confirmation",
    "Human/operator confirmation required",
    "Mission Control remains read-only",
    "Read-only proposal readback",
)

MISSION_CONTROL_FORBIDDEN_ACTION_TOKENS = (
    "Confirm proposal",
    "Run proposal",
    "Execute proposal",
    "Create handoff",
    "Approve proposal",
    "Merge proposal",
    "Cleanup proposal",
    "<button",
    "<form",
    "onSubmit",
    "onClick",
)


class SmokeFailure(RuntimeError):
    """Raised when the smoke path violates its expected safety contract."""


def _require_absolute_path(path: str | Path, field_name: str) -> Path:
    resolved = Path(path).expanduser()
    if not resolved.is_absolute():
        raise ValueError(f"{field_name} must be absolute: {path}")
    return resolved


def _assert_response(response: Any, expected_status: int, action: str) -> dict[str, Any]:
    if response.status_code != expected_status:
        raise SmokeFailure(
            f"{action} returned HTTP {response.status_code}: {response.text}"
        )
    payload = response.json()
    if not isinstance(payload, dict):
        raise SmokeFailure(f"{action} returned non-object JSON: {payload!r}")
    return payload


def _require(condition: bool, message: str) -> None:
    if not condition:
        raise SmokeFailure(message)


def _db_counts(db_path: Path) -> dict[str, int]:
    with sqlite3.connect(db_path) as conn:
        return {
            "tasks": conn.execute("SELECT COUNT(*) FROM tasks").fetchone()[0],
            "events": conn.execute("SELECT COUNT(*) FROM task_events").fetchone()[0],
            "artifacts": conn.execute(
                "SELECT COUNT(*) FROM task_artifacts"
            ).fetchone()[0],
            "worktrees": conn.execute(
                "SELECT COUNT(*) FROM task_worktrees"
            ).fetchone()[0],
        }


def _forbidden_side_effect_counts(db_path: Path) -> dict[str, int]:
    with sqlite3.connect(db_path) as conn:
        artifact_placeholders = ",".join("?" for _ in FORBIDDEN_ARTIFACT_TYPES)
        event_placeholders = ",".join("?" for _ in FORBIDDEN_EVENT_TYPES)
        marker_clause = " OR ".join(
            "payload_json LIKE ?" for _ in FORBIDDEN_PAYLOAD_MARKERS
        )
        artifacts = conn.execute(
            f"""
            SELECT COUNT(*)
            FROM task_artifacts
            WHERE artifact_type IN ({artifact_placeholders})
            """,
            FORBIDDEN_ARTIFACT_TYPES,
        ).fetchone()[0]
        events = conn.execute(
            f"""
            SELECT COUNT(*)
            FROM task_events
            WHERE event_type IN ({event_placeholders})
            """,
            FORBIDDEN_EVENT_TYPES,
        ).fetchone()[0]
        payload_markers = conn.execute(
            f"""
            SELECT COUNT(*)
            FROM task_events
            WHERE payload_json IS NOT NULL
              AND ({marker_clause})
            """,
            tuple(f"%{marker}%" for marker in FORBIDDEN_PAYLOAD_MARKERS),
        ).fetchone()[0]

    return {
        "artifacts": artifacts,
        "events": events,
        "payload_markers": payload_markers,
    }


def _read_json_object(path: Path, label: str) -> dict[str, Any]:
    _require(path.is_file(), f"{label} does not exist: {path}")
    try:
        payload = json.loads(path.read_text(encoding="utf-8"))
    except json.JSONDecodeError as exc:
        raise SmokeFailure(f"{label} is not valid JSON: {path}") from exc
    _require(isinstance(payload, dict), f"{label} JSON is not an object: {path}")
    return payload


def _function_source(source: str, name: str, next_name: str) -> str:
    start = source.find(f"export async function {name}")
    end = source.find(f"export async function {next_name}", start + 1)
    _require(start >= 0, f"Mission Control API helper missing: {name}")
    _require(end > start, f"Mission Control API helper boundary missing: {next_name}")
    return source[start:end]


def _assert_mission_control_read_only_source() -> None:
    api_src = (REPO_ROOT / "mission-control" / "lib" / "api.ts").read_text(
        encoding="utf-8"
    )
    panel_src = (
        REPO_ROOT
        / "mission-control"
        / "components"
        / "SchedulerProposalPanel.tsx"
    ).read_text(encoding="utf-8")

    global_helper = _function_source(
        api_src,
        "getSchedulerProposals",
        "getTaskSchedulerProposals",
    )
    task_helper = _function_source(
        api_src,
        "getTaskSchedulerProposals",
        "getTaskDetailBundle",
    )
    for helper_name, helper_src in (
        ("getSchedulerProposals", global_helper),
        ("getTaskSchedulerProposals", task_helper),
    ):
        _require(
            "requestJson<SchedulerProposalReadback>" in helper_src,
            f"{helper_name} does not use requestJson",
        )
        for token in ("postJson", "POST", "PATCH", "DELETE"):
            _require(
                token not in helper_src,
                f"{helper_name} includes mutation token: {token}",
            )

    for phrase in MISSION_CONTROL_SAFETY_TEXT:
        _require(
            phrase in panel_src,
            f"SchedulerProposalPanel missing safety text: {phrase}",
        )

    for token in MISSION_CONTROL_FORBIDDEN_ACTION_TOKENS:
        _require(
            token not in panel_src,
            f"SchedulerProposalPanel includes action control token: {token}",
        )


def _seed_ready_candidate_task(
    *,
    store: TaskMirrorStore,
    task_key: str,
    project: str,
    repo_path: Path,
    artifact_dir: Path,
) -> None:
    repo_path.mkdir(parents=True, exist_ok=True)
    artifact_dir.mkdir(parents=True, exist_ok=True)
    store.upsert_task(
        TaskRecord(
            task_key=task_key,
            project=project,
            board=project,
            title="Scheduler proposal creation hardening smoke",
            status="queued",
            repo_path=repo_path,
            artifact_dir=artifact_dir,
            created_at="2026-05-01T00:00:00Z",
            updated_at="2026-05-01T00:00:00Z",
        )
    )


def _assert_creation_safety(safety: dict[str, Any]) -> None:
    expected = {
        "proposal_created": True,
        "confirmation_created": False,
        "verifier_report_created": False,
        "handoff_created": False,
        "runtime_started": False,
        "approved_task_runner_called": False,
        "executor_started": False,
        "validators_started": False,
        "github_mutated": False,
        "approved": False,
        "merged": False,
        "cleanup_performed": False,
        "not_execution_permission": True,
    }
    for key, value in expected.items():
        _require(safety.get(key) is value, f"safety.{key} expected {value!r}")


def _assert_readback_item(
    *,
    item: dict[str, Any],
    proposal: dict[str, Any],
) -> None:
    _require(
        item.get("proposal_hash") == proposal.get("proposal_hash"),
        "readback proposal_hash mismatch",
    )
    _require(
        item.get("proposal_item_id") == proposal.get("proposal_item_id"),
        "readback proposal_item_id mismatch",
    )
    _require(
        item.get("item_hash") == proposal.get("item_hash"),
        "readback item_hash mismatch",
    )
    _require(
        item.get("not_execution_permission") is True,
        "readback item is missing not_execution_permission",
    )
    _require(
        item.get("not_confirmation") is True,
        "readback item is missing not_confirmation",
    )
    _require(
        item.get("requires_human_confirmation") is True,
        "readback item is missing requires_human_confirmation",
    )


def run_smoke(
    *,
    workspace_root: Path,
    task_key: str = DEFAULT_TASK_KEY,
    project: str = DEFAULT_PROJECT,
) -> dict[str, Any]:
    """Run the smoke against an isolated workspace root and return a summary."""

    normalized_task_key = normalize_task_key(task_key)
    workspace_root = _require_absolute_path(workspace_root, "workspace_root")
    db_path = workspace_root / "scheduler-proposal-hardening-smoke.db"
    repo_path = workspace_root / "repo"
    artifact_root = workspace_root / "artifacts"
    artifact_dir = artifact_root / normalized_task_key

    workspace_root.mkdir(parents=True, exist_ok=True)
    artifact_root.mkdir(parents=True, exist_ok=True)

    store = TaskMirrorStore(db_path)
    store.init_db()
    _seed_ready_candidate_task(
        store=store,
        task_key=normalized_task_key,
        project=project,
        repo_path=repo_path,
        artifact_dir=artifact_dir,
    )

    result = create_scheduler_proposal_from_candidate(
        SchedulerCandidateProposalRequest(
            task_key=normalized_task_key,
            db_path=db_path,
            artifact_root=artifact_root,
            dry_run=False,
            confirm_create_proposal=True,
            expected_status="queued",
            expected_recommended_command_kind=EXPECTED_COMMAND_KIND,
        )
    )

    _require(result.get("ok") is True, f"proposal creation was not ok: {result!r}")
    _require(result.get("status") == "created", "proposal creation was not created")
    safety = result.get("safety")
    _require(isinstance(safety, dict), "proposal creation safety block missing")
    _assert_creation_safety(safety)

    proposal = result.get("proposal")
    _require(isinstance(proposal, dict), "proposal summary missing")
    proposal_artifact_path = proposal.get("proposal_artifact_path")
    _require(
        isinstance(proposal_artifact_path, str) and proposal_artifact_path,
        "proposal artifact path missing",
    )
    artifact_path = Path(proposal_artifact_path)
    artifact_payload = _read_json_object(artifact_path, "scheduler proposal artifact")
    artifact_items = artifact_payload.get("items")
    _require(isinstance(artifact_items, list), "proposal artifact items is not a list")
    _require(len(artifact_items) == 1, "proposal artifact does not contain one item")
    artifact_item = artifact_items[0]
    _require(isinstance(artifact_item, dict), "proposal artifact item is not an object")

    _require(
        artifact_payload.get("proposal_hash") == proposal.get("proposal_hash"),
        "proposal artifact proposal_hash mismatch",
    )
    _require(
        artifact_item.get("task_key") == normalized_task_key,
        "proposal artifact task_key mismatch",
    )
    _require(
        artifact_item.get("proposal_item_id") == proposal.get("proposal_item_id"),
        "proposal artifact proposal_item_id mismatch",
    )
    _require(
        artifact_item.get("item_hash") == proposal.get("item_hash"),
        "proposal artifact item_hash mismatch",
    )
    _require(
        artifact_item.get("recommended_command_kind") == EXPECTED_COMMAND_KIND,
        "proposal artifact recommended_command_kind mismatch",
    )

    artifacts = [
        artifact
        for artifact in store.list_task_artifacts(normalized_task_key)
        if artifact.artifact_type == PROPOSAL_ARTIFACT_TYPE
    ]
    events = [
        event
        for event in store.list_task_events(normalized_task_key)
        if event.event_type == PROPOSAL_EVENT_TYPE
    ]
    _require(len(artifacts) == 1, f"expected one proposal artifact row, got {len(artifacts)}")
    _require(len(events) == 1, f"expected one proposal event row, got {len(events)}")

    forbidden_counts = _forbidden_side_effect_counts(db_path)
    _require(
        forbidden_counts == {"artifacts": 0, "events": 0, "payload_markers": 0},
        f"forbidden side effects found: {forbidden_counts}",
    )
    _require(
        store.list_executor_runs(normalized_task_key) == [],
        "executor run metadata was recorded",
    )
    _require(
        store.list_validation_results(normalized_task_key) == [],
        "validation result metadata was recorded",
    )
    _require(
        store.list_runtime_audit_events(normalized_task_key) == [],
        "runtime audit metadata was recorded",
    )

    helper_before = _db_counts(db_path)
    helper_readback = list_task_scheduler_proposal_readbacks(
        store,
        normalized_task_key,
    )
    _require(helper_readback.get("count") == 1, "J2 helper did not return one item")
    _require(_db_counts(db_path) == helper_before, "J2 helper mutated DB counts")
    readback_items = helper_readback.get("items")
    _require(isinstance(readback_items, list), "J2 helper items missing")
    helper_item = readback_items[0]
    _require(isinstance(helper_item, dict), "J2 helper item is not an object")
    _assert_readback_item(item=helper_item, proposal=proposal)
    helper_safety = helper_readback.get("safety")
    _require(isinstance(helper_safety, dict), "J2 helper safety block missing")
    _require(helper_safety.get("read_only") is True, "J2 helper is not read-only")
    _require(
        helper_safety.get("proposal_created") is False,
        "J2 helper safety says it created a proposal",
    )

    app = create_app(db_path)
    api_before = _db_counts(db_path)
    with TestClient(app) as client:
        global_payload = _assert_response(
            client.get(
                "/api/scheduler/proposals",
                params={"task_key": normalized_task_key},
            ),
            200,
            "global scheduler proposal readback",
        )
        task_payload = _assert_response(
            client.get(f"/api/tasks/{normalized_task_key}/scheduler-proposals"),
            200,
            "task scheduler proposal readback",
        )
        _assert_response(
            client.get(
                "/api/scheduler/proposals",
                params={"task_key": normalized_task_key},
            ),
            200,
            "repeated global scheduler proposal readback",
        )
        _assert_response(
            client.get(f"/api/tasks/{normalized_task_key}/scheduler-proposals"),
            200,
            "repeated task scheduler proposal readback",
        )

    _require(_db_counts(db_path) == api_before, "J2 API GET mutated DB counts")

    for label, payload in (("global", global_payload), ("task", task_payload)):
        _require(payload.get("count") == 1, f"{label} API count is not one")
        payload_items = payload.get("items")
        _require(isinstance(payload_items, list), f"{label} API items missing")
        payload_item = payload_items[0]
        _require(isinstance(payload_item, dict), f"{label} API item is not an object")
        _assert_readback_item(item=payload_item, proposal=proposal)
        payload_safety = payload.get("safety")
        _require(isinstance(payload_safety, dict), f"{label} API safety missing")
        _require(
            payload_safety.get("not_execution_permission") is True,
            f"{label} API missing top-level not_execution_permission",
        )
        _require(
            payload_safety.get("not_confirmation") is True,
            f"{label} API missing top-level not_confirmation",
        )

    _assert_mission_control_read_only_source()

    return {
        "ok": True,
        "task_key": normalized_task_key,
        "db_path": str(db_path),
        "workspace_root": str(workspace_root),
        "artifact_root": str(artifact_root),
        "proposal": {
            "proposal_id": proposal.get("proposal_id"),
            "proposal_hash": proposal.get("proposal_hash"),
            "proposal_item_id": proposal.get("proposal_item_id"),
            "item_hash": proposal.get("item_hash"),
            "recommended_command_kind": proposal.get("recommended_command_kind"),
            "artifact_path": str(artifact_path),
        },
        "readbacks": {
            "helper_count": helper_readback.get("count"),
            "api_global_count": global_payload.get("count"),
            "api_task_count": task_payload.get("count"),
        },
        "safety": {
            "proposal_created": safety.get("proposal_created"),
            "confirmation_created": safety.get("confirmation_created"),
            "verifier_report_created": safety.get("verifier_report_created"),
            "handoff_created": safety.get("handoff_created"),
            "runtime_started": safety.get("runtime_started"),
            "approved_task_runner_called": safety.get("approved_task_runner_called"),
            "executor_started": safety.get("executor_started"),
            "validators_started": safety.get("validators_started"),
            "github_mutated": safety.get("github_mutated"),
            "approved": safety.get("approved"),
            "merged": safety.get("merged"),
            "cleanup_performed": safety.get("cleanup_performed"),
            "not_execution_permission": safety.get("not_execution_permission"),
        },
        "forbidden_side_effect_counts": forbidden_counts,
    }


def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        description="Run the J4 scheduler proposal creation hardening smoke.",
    )
    parser.add_argument(
        "--task-key",
        default=DEFAULT_TASK_KEY,
        help=f"Task key to use. Default: {DEFAULT_TASK_KEY}",
    )
    parser.add_argument(
        "--workspace-root",
        help=(
            "Absolute workspace root to use. By default a temporary directory "
            "under /tmp is created and removed after the run."
        ),
    )
    parser.add_argument(
        "--keep-workspace",
        action="store_true",
        help="Keep the auto-created temporary workspace after the run.",
    )
    return parser


def main(argv: list[str] | None = None) -> int:
    parser = build_parser()
    args = parser.parse_args(argv)

    cleanup_workspace = False
    workspace_root: Path | None = None
    try:
        if args.workspace_root:
            workspace_root = _require_absolute_path(
                args.workspace_root,
                "workspace_root",
            )
        else:
            workspace_root = Path(
                tempfile.mkdtemp(
                    prefix="agent-taskflow-scheduler-proposal-hardening-",
                    dir="/tmp",
                )
            )
            cleanup_workspace = not args.keep_workspace

        summary = run_smoke(
            workspace_root=workspace_root,
            task_key=args.task_key,
        )
        summary["workspace_kept"] = not cleanup_workspace
        print(json.dumps(summary, indent=2, sort_keys=True))
        return 0
    except Exception as exc:
        print(
            f"Scheduler proposal creation hardening smoke failed: {exc}",
            file=sys.stderr,
        )
        return 1
    finally:
        if cleanup_workspace and workspace_root is not None:
            shutil.rmtree(workspace_root, ignore_errors=True)


if __name__ == "__main__":
    raise SystemExit(main())
