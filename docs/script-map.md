...
| `scripts/migrate_project_class_controls.py` | Apply the additive M1-D project/task-class control-scope migration. | Mutates only the explicitly supplied SQLite DB; preserves controls/events. | Explicit DB path and operator invocation. | No - no background scheduler behavior. |
| `scripts/runtime_control.py` | Inspect or mutate global/project/task-class/task/Attempt controls. | Mutates only the explicitly supplied SQLite DB; every mutation requires `--actor`. | Explicit action, scope, and actor. | No - no background scheduler behavior. |
| `scripts/run_m1_project_class_control_rehearsal.py` | Produce M1-D project admission and class-governance evidence. | Refuses the production DB and exercises a disposable two-project/two-class fixture. | Explicit disposable DB and output paths. | No - no background scheduler behavior. |
