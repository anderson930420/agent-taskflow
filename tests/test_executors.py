...
class ExecutorContextTests(ExecutorTestCase):
    ...
    def test_executor_context_rejects_empty_project(self) -> None:
        with tempfile.TemporaryDirectory() as tmp:
            worktree_path = Path(tmp) / "worktree"
            artifact_dir = Path(tmp) / "artifacts"
            worktree_path.mkdir()
    ...
    def test_executor_context_rejects_non_positive_timeout(self) -> None:
        with tempfile.TemporaryDirectory() as tmp:
    ...
    def test_executor_context_rejects_non_string_env_value(self) -> None:
        with tempfile.TemporaryDirectory() as tmp:
            worktree_path = Path(tmp) / "worktree"
            artifact_dir = Path(tmp) / "artifacts"
            worktree_path.mkdir()
    ...
    def test_executor_context_preserves_valid_env(self) -> None:
        with tempfile.TemporaryDirectory() as tmp:
            worktree_path = Path(tmp) / "worktree"
            artifact_dir = Path(tmp) / "artifacts"
            worktree_path.mkdir()
