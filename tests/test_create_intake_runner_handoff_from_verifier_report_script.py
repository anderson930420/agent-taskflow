"""CLI tests for create_intake_runner_handoff_from_verifier_report.py."""
...
SCRIPT = (
    REPO_ROOT / "scripts" / "create_intake_runner_handoff_from_verifier_report.py"
)
...
class _CliBase(unittest.TestCase):
    ...
    def _seed_task(self, task_key: str, *, status: str = "queued") -> None:
        ...
        self.store.upsert_task(
            TaskRecord(
                ...
                title=f"L5A cli handoff {task_key}",
                status=status,
                repo_path=self.repo,
    ...
    def _create_verifier_report(self, task_key: str) -> dict[str, Any]:
        ...
        report_payload = create_scheduler_confirmation_verifier_report(
            SchedulerConfirmationVerifierReportRequest(
                db_path=self.db_path,
                artifact_root=self.artifact_root,
                task_key=task_key,
        ...
        self.assertTrue(report_payload["ok"], report_payload)
        return report_payload["verifier_report"]
    ...
    def _common_args(self, task_key: str, report: dict[str, Any]) -> list[str]:
        return [
            "--db-path",
            str(self.db_path),
            "--artifact-root",
            ...
            "--proposal-hash",
            report["proposal_hash"],
            "--proposal-item-id",
            report["proposal_item_id"],
            "--item-hash",
            report["item_hash"],
            "--recommended-command-kind",
            report["recommended_command_kind"],
...
class ScriptHelpTests(_CliBase):
    def test_script_help(self) -> None:
        ...
        self.assertIn("intake_runner_handoff", result.stdout)
        self.assertIn("--task-key", result.stdout)
        self.assertIn("--verifier-report-id", result.stdout)
...
class ScriptDryRunTests(_CliBase):
    def test_script_dry_run_writes_nothing(self) -> None:
        task_key = "AT-L5A-CLI-DRY-001"
        report = self._create_verifier_report(task_key)
        before = self._db_counts()

        result = self._run(*self._common_args(task_key, report))
        ...
        self.assertTrue(payload["would_create_handoff"])
        self.assertTrue(payload["binding"]["handoff_allowed"])
...
class ScriptConfirmedTests(_CliBase):
    def test_script_confirmed_mode_creates_handoff(self) -> None:
        task_key = "AT-L5A-CLI-CRT-001"
        report = self._create_verifier_report(task_key)

        result = self._run(
            *self._common_args(task_key, report),
            "--operator",
            "cli-operator",
            "--operator-note",
            "cli-test",
        ...
        artifact_path = Path(payload["handoff"]["artifact_path"])
        ...
        self.assertEqual(on_disk["verifier_report_id"], report["verifier_report_id"])
        self.assertEqual(on_disk["confirmation_id"], report["confirmation_id"])
        self.assertEqual(on_disk["proposal_hash"], report["proposal_hash"])
        self.assertEqual(on_disk["item_hash"], report["item_hash"])
        self.assertTrue(on_disk["handoff_allowed"])
        self.assertTrue(on_disk["requires_runtime_preflight"])
        self.assertFalse(on_disk["approved_task_runner_called"])

        self.assertEqual(self._handoff_counts(task_key), {"artifacts": 1, "events": 1})


class ScriptNotAllowedTests(_CliBase):
    def test_script_not_allowed_returns_nonzero(self) -> None:
        task_key = "AT-L5A-CLI-NA-001"
        report = self._create_verifier_report(task_key)
        before = self._db_counts()

        args = self._common_args(task_key, report)
        idx = args.index("--item-hash")
        args[idx + 1] = "0" * 64

        result = self._run(*args, "--confirm-create-handoff")

        self.assertEqual(result.returncode, 1, result.stdout + result.stderr)
        payload = json.loads(result.stdout)
        self.assertFalse(payload["ok"])
        self.assertEqual(payload["status"], "not_allowed")
