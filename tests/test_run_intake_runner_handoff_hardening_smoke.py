"""Tests for Level 5A intake runner handoff hardening smoke."""
...
SCRIPT = REPO_ROOT / "scripts" / "run_intake_runner_handoff_hardening_smoke.py"
DOC = REPO_ROOT / "docs" / "intake-runner-handoff-hardening-smoke.md"


def _load_smoke_module():
    spec = importlib.util.spec_from_file_location(
        "run_intake_runner_handoff_hardening_smoke",
        SCRIPT,
    )
...
class RunIntakeRunnerHandoffHardeningSmokeTests(unittest.TestCase):
    @classmethod
    def setUpClass(cls) -> None:
        ...
        cls.summary = cls.smoke.run_smoke(
            ...
            task_key="AT-L5A-HANDOFF-SMOKE-TEST",
        )
    ...
    def test_creates_expected_handoff_chain_evidence(self) -> None:
        ...
        self.assertEqual(len(self._task_artifacts(HANDOFF_ARTIFACT_TYPE)), 1)
        self.assertEqual(len(self._task_events(HANDOFF_EVENT_TYPE)), 1)

    def test_handoff_artifact_safety_flags(self) -> None:
        artifact_path = Path(self.summary["handoff"]["artifact_path"])
        self.assertTrue(artifact_path.is_file())
        payload = json.loads(artifact_path.read_text(encoding="utf-8"))
        self.assertEqual(
            payload["handoff_id"],
            self.summary["handoff"]["handoff_id"],
        )
        self.assertEqual(
    ...
    def test_forbidden_side_effect_counts_zero(self) -> None:
        ...
        self.assertTrue(self.summary["safety"]["handoff_created"])
        self.assertTrue(self.summary["safety"]["requires_runtime_preflight"])
        self.assertTrue(self.summary["safety"]["requires_next_gate"])
