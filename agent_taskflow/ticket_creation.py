"""Ticket creation service (SPEC §10, §10.1, §12.1).

The user supplies three things: repository, prompt, priority. Everything else
is derived deterministically by Python here.
...
from agent_taskflow.ticket_store import TicketAllocation, TicketStore, TicketStoreError
...
class TicketCreationResult:
    ...
    def used_ai_title(self) -> bool:
        return self.metadata.title_source == "ai"
...
def create_ticket(
    ...
    *,
    ...
    status = initial_ticket_status(request.blocked_by)
    resolved_repository = repository

    def build(allocation: TicketAllocation) -> TicketRecord:
    ...
    try:
        ticket = store.create_ticket(
            ticket_prefix=repository.ticket_prefix,
            build=build,
            actor=actor,
            event_type=TICKET_CREATED_EVENT,
            message="Ticket created from repository, prompt and priority",
            payload=_creation_payload(request, repository, metadata, status),
            blocked_by=request.blocked_by,
        )
...
def ticket_to_dict(ticket: TicketRecord) -> dict[str, Any]:
    ...
    return {
        ...
        "title": ticket.title,
        "title_source": ticket.title_source,
