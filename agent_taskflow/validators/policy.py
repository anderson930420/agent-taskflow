...
# secret leakage only.
#
# The codex-advisory-review.* artifacts are deterministically generated by
# agent-taskflow's Codex advisory reviewer (not by the bounded worker). They
# enumerate the same governance prohibitions ("no merge", "delete worktree",
# etc.) as documentation, exactly like the mission contract and implementation
# prompt, and the reviewer itself is read-only and never executes worktree
# actions. Treat them as instruction/advisory artifacts: scan for secrets only,
# not for suspicious actions (which would be governance-text false positives).
